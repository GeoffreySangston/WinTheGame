...
Entity.prototype.clone = function(){
	...
	newEntity.imgRef = ent.imgRef;
	newEntity.type = ent.type;
	newEntity.width = ent.width;
	newEntity.height = ent.height;
	newEntity.pw = ent.pw;
	newEntity.ph = ent.ph;
...
Entity.prototype.calcFrameClip = function(){
	var x = this.frame * this.width;
	var y = 0;
...
Entity.prototype.collides = function(oe){
	if(this.collidable && oe.collidable){
		if(this.canCollideWith(oe)){
			if(this.collisionType == Collision.RECTANGLE && oe.collisionType == Collision.RECTANGLE){
...
Entity.prototype.calcPlayerOrthoTheta = function(game){
	...
	var numerator = this.y - (game.playerInfo.playerTurret.y + game.playerInfo.playerTurret.height/2 - this.width/2);
	var denominator = this.x - (game.playerInfo.playerTurret.x + game.playerInfo.playerTurret.width/2 - this.height/2);
...
Entity.prototype.calcCenterOrthoTheta = function(game){
	...
	var gameState = game.stateMachine.curState;
	var numerator = this.y - (screenCenterY - this.width/2);
	var denominator = this.x - (screenCenterX - this.height/2);
	var theta = Math.atan2(numerator, denominator);
...
Entity.prototype.calcPlayerEntityXYVec = function(game){
	var playerCenterX = game.playerInfo.playerTurret.x + game.playerInfo.playerTurret.width/2;
	var playerCenterY = game.playerInfo.playerTurret.y + game.playerInfo.playerTurret.height/2;
...
Entity.prototype.isOnScreen = function(){ // really should rename to isOnScreen
	return (this.x + this.width > 0 && this.x < GAMEWIDTH) && (this.y + this.height > 0 && this.y < GAMEHEIGHT);
};
